...
//    box2D = new SceneWidget(ui->box2DWidget->setEnabled(false););
    ...
    //ui->label->setPixmap(QPixmap(":/images/full.png").scaled(ui->label->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
}
...
void Game::receiveDisplayVector(std::vector<std::vector<int>> dv, std::vector<std::vector<int>> solution)
{
